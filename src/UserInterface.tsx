...
const UserInterface: FC = () => {
  ...
  useEffect(() => {
    async function setAddresses() {
      ...
      const makerBarCoinAssocTokenAcct =
        await spl.Token.getAssociatedTokenAddress(
          spl.ASSOCIATED_TOKEN_PROGRAM_ID,
          spl.TOKEN_PROGRAM_ID,
          barCoinMint,
      ...
      const takerFooCoinAssocTokenAcct =
        await spl.Token.getAssociatedTokenAddress(
          spl.ASSOCIATED_TOKEN_PROGRAM_ID,
          spl.TOKEN_PROGRAM_ID,
          fooCoinMint,
      ...
      const takerBarCoinAssocTokenAcct =
        await spl.Token.getAssociatedTokenAddress(
          spl.ASSOCIATED_TOKEN_PROGRAM_ID,
          spl.TOKEN_PROGRAM_ID,
          barCoinMint,
      ...
      const [escrowAccount, escrowAccountBump] =
        await anchor.web3.PublicKey.findProgramAddress(
  ...
  async function initializeEscrow() {
    ...
    if (payer && program) {
      const tx = new anchor.web3.Transaction()
        ...
        .add(
          ...
          program.instruction.initMakerAssocTokenAccts({
            accounts: {
              fooCoinMint: addresses["fooCoinMint"],
              barCoinMint: addresses["barCoinMint"],
              makerFooCoinAssocTokenAcct:
                addresses["makerFooCoinAssocTokenAcct"],
              makerBarCoinAssocTokenAcct:
                addresses["makerBarCoinAssocTokenAcct"],
              tokenProgram: spl.TOKEN_PROGRAM_ID,
        ...
        .add(
          ...
          program.instruction.initTakerAssocTokenAccts({
            accounts: {
              fooCoinMint: addresses["fooCoinMint"],
              barCoinMint: addresses["barCoinMint"],
              takerFooCoinAssocTokenAcct:
                addresses["takerFooCoinAssocTokenAcct"],
              takerBarCoinAssocTokenAcct:
                addresses["takerBarCoinAssocTokenAcct"],
              tokenProgram: spl.TOKEN_PROGRAM_ID,
              payer: payer.publicKey,
        ...
        .add(
          // Initialize escrow.
          program.instruction.initEscrow(addresses["escrowAccountBump"], {
            accounts: {
              fooCoinMint: addresses["fooCoinMint"],
  ...
  async function submitEscrow() {
    ...
    if (payer && program) {
      ...
      await program.provider.send(
        tx,
        [addresses["maker"], addresses["taker"]],
  ...
  function escrowValid(): boolean {
    return (
      ...
      (
        Boolean(state["fooCoinAmount"]) &&
        (state["fooCoinAmount"] <= state["willFooCoinBalance"])
      ) &&
      (
        Boolean(state["barCoinAmount"]) &&
        (state["barCoinAmount"] <= state["alanBarCoinBalance"])
      )
    );
  }
  ...
  async function handleIxButtonClick(buttonName: string) {
    ...
    if (buttonName === "initialize" && !state["escrowInitialized"]) {
      ...
      setState({
        ...
        willFooCoinBalance:
          state["willFooCoinBalance"] - state["fooCoinAmount"],
      ...
      setState({
        ...
        alanBarCoinBalance:
          state["alanBarCoinBalance"] - state["barCoinAmount"],
      ...
      setState({
        ...initialState,
        escrowInitialized:
          state["submitButtonClicked"] && state["acceptButtonClicked"]
            ? false
            : state["escrowInitialized"],
      });
    }
